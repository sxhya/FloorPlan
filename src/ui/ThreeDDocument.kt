...
import model.WallLayoutKind
import model.AssetDefinition
import java.io.File
...
class ThreeDDocument(val app: FloorPlanApp) {
    ...
    // Embedded floor plan data - name, height, draw flag, and the actual floor document
    class FloorData(var name: String, var height: Int, var draw: Boolean = true, val floorDoc: FloorPlanDocument)
    val floors = mutableListOf<FloorData>()

    // House-level kinds and asset definitions (shared across all embedded floors)
